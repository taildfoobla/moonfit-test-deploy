...
const CopySvg = () => (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M13.125 13.125H16.875V3.125H6.875V6.875" stroke="#4CCBC9" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M13.125 6.875H3.125V16.875H13.125V6.875Z" stroke="#4CCBC9" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
);
...
const RewardModal = (props) => {
    ...
    const _getRewardValue = (type, value, imageUrl = "") => {
        ...
        switch (type) {
            ...
            case "MFG":
            case "GLMR":
                output = <h1 className="text-triple-gradient-2">{value}</h1>
                break;
            case "Mug":
            case "T-shirt":
            case "Hoodie":
    ...
    return (
        <div className="task-reward">
            <h2 className="text-triple-gradient-2">YOU WIN</h2>
            {_getRewardValue(reward?.type, reward?.value, reward?.image_url)}
            <h3 className="text-pink-glow">{["MFR", "MFG", "GLMR"].includes(reward?.type) ? `$${reward?.type}` : `01 ${reward?.type}`}</h3>
            {
                !["MFR", "MFG", "GLMR"].includes(reward?.type) && <div className="button-container">
                    <Paragraph className={"text-primary text-center"}
                        copyable={{
                            text: user.account, format: "text/plain",
